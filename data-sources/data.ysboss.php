...
';
		...
		function about(){
			$params = array();
			foreach (datasourceYSBOSS::__getParamFilters() as $k => $v) {
		...
		function grab($param=array()){
			...
			$p = '?format=xml';
			$p .= '&appid='.urlencode($this->dsParamFILTERS['appid']);

			$count = $this->dsParamFILTERS['count'];
			if (!$count) $count = 10;
			...
			if (trim($temp = $this->dsParamFILTERS['sites'])) $p .= '&sites='.urlencode($temp);
